...
function trap_ctrlc ()
{
    echo "Ctrl-C or Error caught... performing cleanup. Check /tmp/inference_log"
...
# Derive JSON output path automatically from PNG output path
output_json="${output_file%.png}.json"
...
python3 ./inference.py \
    ...
    --mean "$XBDIR"/weights/mean.npy \
    --output "$label_temp"/"${input_image%.*}".json >> "$LOGFILE" 2>&1
...
python3 ./process_data_inference.py \
    --input_img "$disaster_post_file" \
    --label_path "$label_temp"/"${input_image%.*}".json \
...
python3 ./damage_inference.py \
    ...
    --output_json /tmp/inference/classification_inference.json >> "$LOGFILE" 2>&1

printf "\n" >> "$LOGFILE"

printf "Formatting json and scoring image\n"
python3 "$XBDIR"/utils/combine_jsons.py \
    --polys "$label_temp"/"${input_image%.*}".json \
    --classes /tmp/inference/classification_inference.json \
    --output "$inference_base/inference.json" >> "$LOGFILE" 2>&1

printf "\n" >> "$LOGFILE"

printf "Finalizing output file\n"
python3 "$XBDIR"/utils/inference_image_output.py \
    --input "$inference_base"/inference.json \
    --output "$output_file" >> "$LOGFILE" 2>&1

# Save the final merged inference JSON next to the PNG
cp "$inference_base/inference.json" "$output_json"
