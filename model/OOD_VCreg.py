...
OUTPUT_DIR = BASE_DIR / "OOD_training_outputs" / "baseline_resnet50_vicreg"
...
# VICReg style representation regularization
VICREG_WEIGHT = 0.01
...
# Model
# =========================
class ResNet50SixChannelVICReg(nn.Module):
...
# =========================
# VICReg style regularizer
# =========================
def vicreg_feature_loss(features, variance_target=1.0):
    ...
    variance_loss = torch.mean(F.relu(variance_target - std))

    covariance = (features.T @ features) / (features.size(0) - 1)
    covariance_off_diag = covariance - torch.diag(torch.diag(covariance))
    covariance_loss = (covariance_off_diag ** 2).sum() / features.size(1)
...
def main():
    ...
    model = ResNet50SixChannelVICReg(num_classes=4).to(device)
    ...
    print("\nStarting OOD training with VICReg style representation regularization...")
    print(f"VICReg weight: {VICREG_WEIGHT}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for x, y in progress_bar:
            ...
            variance_loss, covariance_loss = vicreg_feature_loss(
                features,
            ...
            loss = ce_loss + VICREG_WEIGHT * regularization_loss
            ...
            total_variance_loss += variance_loss.item() * x.size(0)
            total_covariance_loss += covariance_loss.item() * x.size(0)

            progress_bar.set_postfix(
                loss=f"{loss.item():.4f}",
                ce=f"{ce_loss.item():.4f}",
                var=f"{variance_loss.item():.4f}",
                cov=f"{covariance_loss.item():.4f}",
            )

        train_loss = total_loss / len(train_loader.dataset)
        train_ce_loss = total_ce_loss / len(train_loader.dataset)
        train_variance_loss = total_variance_loss / len(train_loader.dataset)
        train_covariance_loss = total_covariance_loss / len(train_loader.dataset)
        ...
        history.append(
            {
                ...
                "train_cross_entropy_loss": train_ce_loss,
                "train_variance_loss": train_variance_loss,
                "train_covariance_loss": train_covariance_loss,
        ...
        print(
            ...
            f"Var: {train_variance_loss:.4f} | "
            f"Cov: {train_covariance_loss:.4f} | "
    ...
    torch.save(best_state, OUTPUT_DIR / "best_model_vicreg.pt")

    history_df = pd.DataFrame(history)
    history_df.to_csv(OUTPUT_DIR / "training_history_vicreg.csv", index=False)
    ...
    np.save(OUTPUT_DIR / "ood_test_preds_vicreg.npy", np.array(final_test["preds"]))
    np.save(OUTPUT_DIR / "ood_test_targets_vicreg.npy", np.array(final_test["targets"]))
    np.save(OUTPUT_DIR / "ood_hold_preds_vicreg.npy", np.array(final_hold["preds"]))
    np.save(OUTPUT_DIR / "ood_hold_targets_vicreg.npy", np.array(final_hold["targets"]))
    ...
    summary = {
        ...
        "method": "ResNet50 six channel OOD baseline with supervised cross entropy and VICReg style variance and covariance representation regularization on the 2048 dimensional feature vector.",
    }

    with open(OUTPUT_DIR / "summary_vicreg.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    with open(OUTPUT_DIR / "ood_test_classification_report_vicreg.json", "w", encoding="utf-8") as f:
        json.dump(test_report, f, indent=2)

    with open(OUTPUT_DIR / "ood_hold_classification_report_vicreg.json", "w", encoding="utf-8") as f:
        json.dump(hold_report, f, indent=2)

    print("\nSaved all OOD VICReg training outputs successfully.")
    print(OUTPUT_DIR / "best_model_vicreg.pt")
    print(OUTPUT_DIR / "training_history_vicreg.csv")
    print(OUTPUT_DIR / "summary_vicreg.json")
