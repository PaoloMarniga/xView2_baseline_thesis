...
OUTPUT_DIR = BASE_DIR / "OOD_training_outputs" / "resnet50_supervised_contrastive_decorellation"
...
def decorrelation_loss(z):
    """
    Encourages projection dimensions to be less redundant.

    z shape:
    batch_size x projection_dim
    """
    ...
    corr = torch.matmul(z.T, z) / (z.size(0) - 1)

    identity = torch.eye(corr.size(0), device=z.device)
...
def main():
    ...
    print("\nStarting ResNet50 supervised contrastive pretraining with decorrelation loss...")
    print(f"Decorrelation weight: {DECORRELATION_WEIGHT}")
    ...
    for epoch in range(CONTRASTIVE_EPOCHS):
        ...
        for x1, x2, y in progress_bar:
            ...
            supcon = contrastive_criterion(projections, y_contrastive)
            decor = decorrelation_loss(projections)

            loss = supcon + DECORRELATION_WEIGHT * decor
            ...
            total_loss += loss.item() * x.size(0)
            total_supcon_loss += supcon.item() * x.size(0)
            total_decorrelation_loss += decor.item() * x.size(0)

            progress_bar.set_postfix(
                loss=f"{loss.item():.4f}",
                supcon=f"{supcon.item():.4f}",
                decor=f"{decor.item():.4f}",
            )

        epoch_minutes = (time.time() - epoch_start) / 60.0
        denom = len(contrastive_loader.dataset) * 2

        avg_loss = total_loss / denom
        avg_supcon_loss = total_supcon_loss / denom
        avg_decorrelation_loss = total_decorrelation_loss / denom

        contrastive_history.append(
            {
                "epoch": epoch + 1,
                "contrastive_total_loss": avg_loss,
                "supervised_contrastive_loss": avg_supcon_loss,
                "decorrelation_loss": avg_decorrelation_loss,
        ...
        print(
            ...
            f"Decor: {avg_decorrelation_loss:.4f} | "
        ...
        torch.save(
            ...
            OUTPUT_DIR / "contrastive_decorellation_latest.pt",
        )
    ...
    torch.save(
        ...
        OUTPUT_DIR / "contrastive_decorellation_final.pt",
    )

    pd.DataFrame(contrastive_history).to_csv(
        OUTPUT_DIR / "contrastive_decorellation_history.csv",
        index=False,
    )

    print("\nStarting classifier training on contrastive plus decorrelation ResNet50 encoder...")
    ...
    torch.save(best_state, OUTPUT_DIR / "best_model_decorellation.pt")

    pd.DataFrame(classifier_history).to_csv(
        OUTPUT_DIR / "classifier_decorellation_history.csv",
        index=False,
    )

    print("\nEvaluating best ResNet50 supervised contrastive plus decorrelation model...")
    ...
    np.save(OUTPUT_DIR / "ood_test_preds_decorellation.npy", np.array(final_test["preds"]))
    np.save(OUTPUT_DIR / "ood_test_targets_decorellation.npy", np.array(final_test["targets"]))
    np.save(OUTPUT_DIR / "ood_hold_preds_decorellation.npy", np.array(final_hold["preds"]))
    np.save(OUTPUT_DIR / "ood_hold_targets_decorellation.npy", np.array(final_hold["targets"]))
    np.save(OUTPUT_DIR / "ood_test_confusion_matrix_decorellation.npy", test_cm)
    np.save(OUTPUT_DIR / "ood_hold_confusion_matrix_decorellation.npy", hold_cm)
    ...
    summary = {
        ...
        "method": "ResNet50 six-channel encoder with supervised contrastive representation pretraining plus decorrelation loss, followed by classifier training without class weights.",
    }

    with open(OUTPUT_DIR / "summary_decorellation.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    with open(OUTPUT_DIR / "ood_test_classification_report_decorellation.json", "w", encoding="utf-8") as f:
        json.dump(test_report, f, indent=2)

    with open(OUTPUT_DIR / "ood_hold_classification_report_decorellation.json", "w", encoding="utf-8") as f:
        json.dump(hold_report, f, indent=2)

    print("\nSaved all ResNet50 supervised contrastive plus decorrelation outputs successfully.")
    print(OUTPUT_DIR / "best_model_decorellation.pt")
    print(OUTPUT_DIR / "classifier_decorellation_history.csv")
    print(OUTPUT_DIR / "summary_decorellation.json")
